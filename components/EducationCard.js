    ...
    <div className="card">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2 sm:gap-0 mb-3 sm:mb-4">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs sm:text-sm text-primary-600 mb-1">{getCourseTypeLabel(courseType)}</p>
          <p className="text-xs sm:text-sm text-gray-600 mb-1 break-words">{schoolOrCollege}</p>
          <p className="text-xs sm:text-sm text-gray-500 break-words">
            {boardOrUniversity} • {passingYear} {isExpected && '(Expected)'}
          </p>
        </div>
        {verified && (
