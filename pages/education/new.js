...
export default function NewEducation({ showToast }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showToast(error.response?.data?.message || 'Failed to create education entry', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <ProtectedRoute>
      <Head>
        <title>Add Education - TruePortMe</title>
      </Head>

      <div className="min-h-screen bg-gray-50">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="mb-8">
            <h1 className="text-2xl font-bold text-gray-900">Add Education Entry</h1>
            <p className="text-gray-600">Share details about your educational background</p>
          </div>
